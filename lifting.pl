...
my $start_dt = (
         string_to_dt( $query->param( 'start_date' ) )
      or string_to_dt( sprintf( "%04d/%02d/%02d", $start_yr, $start_month, $start_day ) )
);
my $stop_dt =
  ( string_to_dt( $query->param( 'stop_date' ) ) or DateTime->now->set( hour => 0, minute => 0, second => 0, ) );
...
if ( $action eq "add" ) {
    ...
    if ( !$dt ) {
        ...
        $sql_statement = "SELECT date FROM ${prefix}lifting WHERE date = ? and type = ?";
        $sth = $dbh->prepare( $sql_statement ) or die "Could not prepare: " . $dbh->errstr();
        $sth->execute( $dt->ymd( '/' ), $type ) or die "Error: " . $dbh->errstr();
        if ( $sth->fetchrow_array() ) {
            my %row = ( error => "An entry for this date already exists, only one PR entry per type allowed per day." );
            push( @error_strings, \%row );
        }
    }
    if ( $weight !~ /^[0-9]{1,3}(\.[0-9]{1,2})?$/ or $weight <= '0' ) {
        my %row =
          ( error =>
"The weight value cannot be empty or negative.  It cannot contain more than three digits before the decimal point or more than two digits after."
          );
        push( @error_strings, \%row );
    }
    ...
    if ( $error_size == 0 ) {
        $sql_statement = "INSERT INTO ${prefix}lifting ( date, weight, type ) VALUES ( ?, ?, ? )";
        $sth = $dbh->prepare( $sql_statement ) or die "Could not prepare: " . $dbh->errstr();
        $sth->execute( $dt->ymd( '/' ), $weight, $type ) or die "Error: " . $dbh->errstr();
    ...
    if ( $query_value !~ /^[0-9]{1,3}(\.[0-9]{1,2})?$/ or $query_value <= 0 ) {
        $error_str .=
'The weight value must be an integer greater than zero and containing no more than three digits before the decimal point or more than two digits after.';
    }
    ...
    if ( $error_str eq 'Error ' ) {
        ...
        $sql_statement = "SELECT date, weight, trend FROM ${prefix}weights WHERE date = ?";
        $sth = $dbh->prepare( $sql_statement ) or die "Could not prepare: " . $dbh->errstr();
        $sth->execute( $prev_dt->ymd( '/' ) ) or die "Could not execute: " . $dbh->errstr();

        if ( my @ary = $sth->fetchrow_array() ) {
            $trend         = ( $ary[2] + $trend_mod * ( $query_value - $ary[2] ) );
            $sql_statement = "UPDATE ${prefix}weights SET weight = ?, trend = ? WHERE id = ?";
            $sth           = $dbh->prepare( $sql_statement ) or die "Could not prepare: " . $dbh->errstr();
            $sth->execute( $query_value, $trend, $id ) or die "Could not execute: " . $dbh->errstr();
        }
        else {
            $trend         = $query_value;
            $sql_statement = "UPDATE ${prefix}weights SET weight = ?, trend = ? WHERE id = ?";
        ...
        $sql_statement = "SELECT date, weight, trend, id FROM ${prefix}weights WHERE date > ? ORDER BY date ASC";
        $sth = $dbh->prepare( $sql_statement ) or die "Could not prepare: " . $dbh->errstr();
        $sth->execute( $dt->ymd( '/' ) ) or die "Could not execute: " . $dbh->errstr();

        while ( my @ary = $sth->fetchrow_array() ) {
            $trend = ( $trend + $trend_mod * ( $ary[1] - $trend ) );
            my $sql_statement2 = "UPDATE ${prefix}weights SET trend = ? WHERE id = ?";
            my $sth2 = $dbh->prepare( $sql_statement2 ) or die "Could not prepare: " . $dbh->errstr();
            $sth2->execute( $trend, $ary[3] ) or die "Could not execute: " . $dbh->errstr();
        }
    ...
    if ( $error_size == 0 ) {
        ...
        $sql_statement = "SELECT date,weight FROM ${prefix}weights WHERE id = ?";
...
$sql_statement = "SELECT date, weight,id FROM ${prefix}lifting WHERE date >= ? AND date <= ? ORDER BY date DESC";
$sth = $dbh->prepare( $sql_statement ) or die "Could not prepare: " . $dbh->errstr();
$sth->execute( $start_dt->ymd( '/' ), $stop_dt->ymd( '/' ) ) or die "Could not execute: " . $dbh->errstr();

my @loop     = ();
my $continue = '1';
while ( $continue and ( my @ary1 = $sth->fetchrow_array() ) ) {
    my %row = (
        date1   => substr( $ary1[0], 5 ),
        weight1 => $ary1[1],
        id      => $ary1[2],
    );
    if ( my @ary2 = $sth->fetchrow_array() ) {
        $row{'date2'}   = substr( $ary2[0], 5 );
        $row{'weight2'} = $ary2[1];
        $row{'id2'}     = $ary2[2],;
    }
    else {
...
$template->param( weight_loop => \@loop );
